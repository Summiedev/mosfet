...
export default function LoginPage() {
    ...
    const handleSignIn = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Save token & user for the rest of the app
            localStorage.setItem('access_token', data.access_token)
            localStorage.setItem('user', JSON.stringify(data.user))

            // Send OTP to the user's phone then go to verify screen
            await authApi.sendOtp(data.user.phone)
                ...
                <div className="bg-[#0F172B] backdrop-blur border border-slate-700/50 rounded-2xl p-8 space-y-8">
                    ...
                    <form onSubmit={handleSignIn} className="space-y-4">
                        ...
                        <Button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#4F39F6] hover:opacity-90 text-white font-medium py-2 rounded-lg transition-all"
                        >
